...
// app.get('/listarlivros/:id/:genero', (req, res) => {
...
app.get('/buscarlivro', async (req, res) => {
    ...
    const {data, error} = await supabase.from('biblioteca_livro').select('*')
    .ilike('name', `%${name}%`).ilike('genero', `%${genero}%`)
...
// A diferença de put e patch é que o put substitui toda a informação do registro, ou seja, se um campo não for enviado, ele será apagado. Já o patch atualiza apenas os campos enviados, mantendo os outros campos inalterados.
// app.put()
// app.path()
